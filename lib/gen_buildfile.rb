...
# TODO: Users of archie (ch.elexis.archie.wzw,ch.shenzi.arztleistung,ch.elexis.pdfBills,) don't compile (at least with OpenJDK)
...
ProjectHeader =<<EOF
THIS_VERSION = '2.1.7.dev' # This will make it smaller than 2.1.7.0, 2.1.7.rc0, etc
...
AddedCommands['dokumentation'] = <<EOF
    ...
    end if false # unless Wikitext::skipDoc
EOF
...
AddedCommands['ch.rgw.utility'] = <<EOF
    ...
    check package(:plugin), 'plugin should not be bigger than 1900 kBytes' do
          File.size(package(:plugin).to_s).should_not > 1900*1024
    end
    ...
    check package(:plugin), 'plugin should contain ch/rgw/compress/CompEx.class' do
      it.should contain('ch/rgw/compress/CompEx.class')
    end
    check package(:plugin), 'plugin should not contain bin/ch/rgw/compress/CompEx.class' do
      it.should_not contain('bin/ch/rgw/compress/CompEx.class')
    end
    ...
    check package(:plugin), 'plugin should not contain bin/ch/rgw/tools/messages_fr.properties' do
      it.should_not contain('bin/ch/rgw/tools/messages_fr.properties')
    end
    # TODO:
    check package(:plugin), 'plugin should not contain src/ch/rgw/compress/CompEx.java' do
      it.should_not contain('src/ch/rgw/compress/CompEx.java')
    end if false
    # TODO:
    check package(:plugin), 'plugin should not contain target/classes/ch/rgw/compress/CompEx.class' do
      it.should_not contain('target/classes/ch/rgw/compress/CompEx.class')
    end if false
EOF
...
def handleOneProject(aProjDir)
  ...
  if @@skipPlugins.index(aProjDir)
    msg = "# Skipping #{aProjDir} as found in #{@@skipName}"
    $buildfile.puts msg
    return
  end
  if $allProjects and $allProjects.index(sName)
    puts "Duplicate project #{sName}"
    exit 3
  end
  if pluginsThatDontWorkYet(sName)
    $buildfile.puts "# #{sName} DontWorkYet. See #{@@skipName}"
    return
  end
...
#  version = '4.5.6'
  ...
  allDeps = []
  allDeps += projDeps if projDeps.size > 0
  localJars = Dir.glob("#{aProjDir}/*.jar") + Dir.glob("#{aProjDir}/lib/*.jar")
  if localJars.size > 0
    $buildfile.puts "    check package(:plugin), 'plugin should include #{localJars.size} local jars' do"
    $buildfile.puts "      it.should exist"
    $buildfile.puts "      it.should contain('META-INF/MANIFEST.MF')" 
    localJars.each{ |jar| dest = jar.sub(aProjDir+'/', '')
                    # Workaround some bugs ch.elexis.hl7.v2x has old jar, but not in build.properties
                    # juh.jar is missing in ch.rgw.OOWrapper, aber wer bracht das noch??
                    # diese Jar scheinen keine gültigen Manifest.mf zu haben.
                    # juh.jar ch.rgw.OOWrapper
                    # jdbc4d.jar ch.elexis.importer.praxisdesktop
                    # commons-httpclient-3.0.1.jar bei ch.medelexis.galexis

      $buildfile.puts "      it.should contain('#{dest}')" unless
                  /ch.elexis.hl7.v2x|ch.elexis.scripting.scala/.match(sName)
                }
    $buildfile.puts "    end"
  end
  ...
  $buildfile.puts "    dependencies << projects('#{allDeps.uniq.join("', '")}')" if allDeps.length > 0
...
Dir.glob("**/buildfile.project").sort.each{ |buildFragment|
  ...
  $buildfile.puts "   fullName = File.expand_path('#{buildFragment}')"
  $buildfile.puts "   # ruby hack: see http://erniemiller.org/2012/09/10/ruby-tidbit-__line__-and-heredocs/"
  $buildfile.puts "   instance_eval(File.read(fullName), fullName)"
  $buildfile.puts "   # load '#{buildFragment}' # does not work because it generates a new context"
  $buildfile.puts " end"
  }

# Find all Eclipse project in all subdirectories (up to 3 levels)
(Dir.glob("*/.project")+Dir.glob("*/*/.project")+Dir.glob("*/*/*/.project")).sort.each{
  |y| 
