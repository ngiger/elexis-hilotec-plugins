...
prerequisites.each {
 |x|
  unless  system("#{x} --version".sub('java --version', 'java -version'))
...
$usePrefix = 'jruby -S'
if $onWindows
  unless system('jruby --version', MayFail)
  ...
  unless system("#{$usePrefix}jruby --version", MayFail) or Dir.glob(jrubyDir).size == 0
...
{'net-ldap' => nil, 'buildrdeb' => nil,'buildrizpack' =>'0.2.1','buildr' => '1.4.7'}.each do
  |name, version|
    unless checkGem(name,version)
...
buildr4osgiInstalled = checkGem('buildr4osgi', '0.9.6.96')

if needsRebuild || Dir.glob(buildr4osgiPath).size == 0 || !buildr4osgiInstalled
  puts "Adding buildr4osgi (special from niklaus)"
  system("git clone git://github.com/ngiger/buildr4osgi.git #{buildr4osgiPath}") unless File.directory?(buildr4osgiPath)
  saved = Dir.pwd
