...
FileUtils.makedirs('clones') unless File.directory?('clones')

system("svn co --quiet #{Archie} clones/svn/ch.unibe.iam.scg.archie") unless File.directory?('clones/svn')
HgRepos.each{ |aRepo|
              base = "#{BaseDir}/clones/#{File.basename(aRepo)}"
              system("hg clone #{aRepo} #{base}")unless File.directory?(base)
            Dir.chdir(base)
